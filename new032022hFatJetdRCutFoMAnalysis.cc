...
void new032022hFatJetdRCutFoMAnalysis(){
    // Open the file. Note that the name of your file outside this class
    // will probably NOT be experiment.root.
    std::cout << "start hFatJetdRCut FoM Analysis\n";
    clock_t startt = clock();
    //Analyzer will run over all files put into fileAr

    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        hFatJetPassAr.push_back(0);
        hFatJetCutRangeAr.push_back(hFatJetdRRangeBottom+(rangeItr*hFatJetdRCutStep));
    }
    ...
    */
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            //tmpZ1Vec.clear();
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                float tmphFatJetdRCutVal = hFatJetCutRangeAr[rangeItr];
                ...
                doVBFJetCut(nJetLen,Jet_ptL,Jet_jetIdL,Jet_etaL,Jet_phiL,Jet_massL,jetPTCut,jetEtaDifCut,jetInvMassCut,jetPairInvMass,jetLeadPt,jetLeadEta,jetLeadPhi,jetTrailingPt,jetTrailingEta,jetTrailingPhi,leadJet_1,leadJet_2,selectedFJ_phi,selectedFJ_eta,debug);
    ...
    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        std::cout << "hFatJetCutRangeAr[rangeItr] " << hFatJetCutRangeAr[rangeItr] << " hFatJetPassAr[rangeItr] " << hFatJetPassAr[rangeItr] << "\n";
    }
