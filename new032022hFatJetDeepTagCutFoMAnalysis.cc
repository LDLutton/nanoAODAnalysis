...
void new032022hFatJetDeepTagCutFoMAnalysis(string datasetString){
    ...
    */
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            //tmpZ1Vec.clear();
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                float tmphFatJetDeepTagCutVal = cutRangeAr[rangeItr];
                ...
                findLepIDBools(elecIDVec,neLep,Electron_mvaFall17V2Iso_WP80L);  
                //findElecIDBools(elecIDVec,neLep,Electron_mvaFall17V2Iso_WP80L);    
                std::vector<Bool_t> muonIDVec;
                findLepIDBools(muonIDVec,nmLep,Muon_tightIdL); 
