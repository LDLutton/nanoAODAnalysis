...
void new032022SemiLepFatJetZPlusHInvMassCutFoMAnalysis(string datasetString){
    // Open the file. Note that the name of your file outside this class
    // will probably NOT be experiment.root.
    std::cout << "start SemiLepFatJetPTCut FoM Analysis\n";
    ...
    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        ...
        cutRangeAr.push_back(fatJetZPlusHInvMassRangeBottom+(rangeItr*fatJetZPlusHInvMassCutStep));
    }
    ...
    */
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                float tmpFatJetZPlusHInvMassCutVal = cutRangeAr[rangeItr];
                Int_t FJInd;
                ...
                if ((!passesCutsBool)){
                    ...
                    */
                    ...
                    ROOT::Math::PtEtaPhiMVector HFJVec = ROOT::Math::PtEtaPhiMVector(hFatJet_pt_fromHTag, hFatJet_eta_fromHTag, hFatJet_phi_fromHTag, hFatJet_mass_fromHTag);
                    ROOT::Math::PtEtaPhiMVector ZFJVec = ROOT::Math::PtEtaPhiMVector(FatJet_ptL[FJInd],selectedFJ_eta[1],selectedFJ_phi[1],FatJet_massL[FJInd]);
                    ROOT::Math::PtEtaPhiMVector HFJPlusZFJVec = HFJVec+ZFJVec;
                    ...
                    Electron_mvaFall17V2Iso_WP80L,Muon_tightIdL,
                    HFJPlusZFJVec,tmpFatJetZPlusHInvMassCutVal,
                    debug);
                }
