...
void new062022HiggsFJCutsFoMAnalysis(string inputString){
    ...
    if (cutString == "hFatJetParticleNet") {
        cutRangeBottom = hFatJetParticleNetRangeBottom;
        cutStep = hFatJetParticleNetCutStep;
        cutPtr = &hFatJetParticleNetCut;
    }
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            */
            ...
            std::vector<Int_t> oldFJIndAr = FJIndAr;
            //std::cout << "------------------------\n"; 
            //std::cout << "before loop FJIndAr.size() " << FJIndAr.size() << "\n";
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
            ...
            //std::cout << "FJIndAr.size() " << FJIndAr.size() << "\n";
