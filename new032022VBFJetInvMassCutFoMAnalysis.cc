...
void new032022VBFJetInvMassCutFoMAnalysis(string datasetString){
    // Open the file. Note that the name of your file outside this class
    // will probably NOT be experiment.root.
    std::cout << "start VBFJetInvMassCut FoM Analysis\n";
    ...
    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        ...
        cutRangeAr.push_back(jetInvMassRangeBottom+(rangeItr*jetInvMassCutStep));
    }
    ...
    */
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                float tmpVBFJetInvMassCutVal = cutRangeAr[rangeItr];
                //Now match VBF jets
                ...
                //Replacing jetPairInvMass
                doVBFJetCut(nJetLen,Jet_ptL,Jet_jetIdL,Jet_etaL,Jet_phiL,Jet_massL,jetPTCut,jetEtaDifCut,tmpVBFJetInvMassCutVal,jetPairInvMass,jetLeadPt,jetLeadEta,jetLeadPhi,jetTrailingPt,jetTrailingEta,jetTrailingPhi,leadJet_1,leadJet_2,selectedFJ_phi,selectedFJ_eta,VBFJetdRCut,debug);
