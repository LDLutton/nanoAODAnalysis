...
void new032022SemiLepFatJetZTagCutFoMAnalysis(string datasetString){
    ...
    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        ...
        cutRangeAr.push_back(fatJetZTagRangeBottom+(rangeItr*fatJetZTagCutStep));
    }
    ...
    */
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                float tmpFatJetZTagCutVal = cutRangeAr[rangeItr];
                Int_t FJInd;
                ...
                if ((!passesCutsBool)){
                    ...
                    //Replacing fatJetZTagCut
                    //std::cout << rangeItr << " " << tmpFatJetPTCutVal << "\n";
                    doSemiLepChanFatJetCut(FJInd,numFatJet,hFatJet_ind_fromHTag,fatJetPTCut,tmpFatJetZTagCutVal,FatJet_ptL,FatJet_phiL,FatJet_etaL,FatJet_deepTag_ZvsQCDL,selectedFJ_phi,selectedFJ_eta);
                    /*
