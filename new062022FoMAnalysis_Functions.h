...
void getNTightAndLooseElectrons(TTreeReaderArray<Bool_t> &Lep_IdL,TTreeReaderArray<Float_t> &Lep_eta,TTreeReaderArray<Float_t> &Lep_pt,TTreeReaderArray<Float_t> &Lep_Dxy,TTreeReaderArray<Float_t> &Lep_Dz,TTreeReaderArray<Float_t> &Lep_sip3d,TTreeReaderArray<Float_t> &Lep_MiniPFRelIso,TTreeReaderArray<Float_t> &Electron_sieie,TTreeReaderArray<Float_t> &Electron_hoe,TTreeReaderArray<Float_t> &Electron_eInvMinusPInv,TTreeReaderArray<Bool_t> &Electron_convVeto,TTreeReaderArray<UChar_t> &Electron_lostHits,TTreeReaderArray<Int_t> &Lep_jetIdx,TTreeReaderArray<Float_t> &Jet_btagDeepFlavB,TTreeReaderArray<Float_t> &Lep_mvaTTH,UInt_t nlLep,float lPtTightCut,float lPtLooseCut,float lEtaCut,float lDxyCut,float lDzCut,float lMiniPFRelIsoCut,float eSieieBarrelCut,float eSieieEndcapCut,float eHoeCut,float eInvMinusPInvCut,UInt_t eLostHitsCut,float lepJetDeepTagLooseCut,float lepJetDeepTagMediumCut,float lPromptMVACut,float lSIP3DCut,UInt_t &eTightCount,UInt_t &eLooseCount,bool debug){
    for (UInt_t lItr=0; lItr < nlLep; lItr++){
        
        if (debug) std::cout << "lItr " << lItr<< "Lep_IdL[lItr]" << Lep_IdL[lItr] << "\n";
        if (debug) std::cout << "eTightCount " << eTightCount<< " eLooseCount " << eLooseCount << "\n";
        bool lepPassTight = false;
        bool lepPassLoose = false;
        if (!Lep_IdL[lItr]){
            continue;
        }
        float tmpEta = Lep_eta[lItr];

        if (debug) std::cout << "lItr " << lItr << " tmpEta " <<tmpEta << "\n";
                ...
                if (abs(tmpDz) < lDzCut) {
                    ...
                    if (tmpLepSIP3D < lSIP3DCut) {
                        ...
                        if (tmpLep_MiniPFRelIso < lMiniPFRelIsoCut*tmpPt) {
                            ...
                            //Now Trying to pass the loose cuts
                            UChar_t tmpLostHits =  Electron_lostHits[lItr];
                            if (tmpLostHits <= eLostHitsCut) {
                                if (debug) std::cout << "passed loose elec misshits cut\n";
                                
                                if (tmpPt > lPtLooseCut){
                                    if (debug) std::cout << "passed loose pt cut and all loose cuts\n";
                                    lepPassLoose = true;
                                    //Now trying tight cuts
                                    if (tmpLostHits <= eLostHitsTightCut) {
                                        if (debug) std::cout << "passed tight elec misshits cut\n";
                                        if (tmpPt > lPtTightCut){
                                            ...
                                            if (tmpLepMVA > lPromptMVACut) {
                                                ...
                                                //Now trying to pass the tight cuts
                                                

                                                if (debug) std::cout << "tmpPt " << tmpPt << "\n";
                                                        ...
                                                        if (tmpHoe < eHoeCut) {
                                                            ...
                                                            if (tmpInvEInvP > eInvMinusPInvCut) {
                                                                ...
                                                                if (Electron_convVeto[lItr]) {
                                                                    if (debug) std::cout << "passed Elec Conv cut and all tight cuts\n";
                                                                    lepPassTight = true;
                                                                }
                                                            }
                                                        }
        ...
        else if (lepPassLoose){
            eLooseCount += 1;
        }
    }
}

void getNTightAndLooseMuons(TTreeReaderArray<Bool_t> &Lep_LooseIdL,TTreeReaderArray<Bool_t> &Lep_MediumIdL,TTreeReaderArray<Float_t> &Lep_eta,TTreeReaderArray<Float_t> &Lep_pt,TTreeReaderArray<Float_t> &Lep_Dxy,TTreeReaderArray<Float_t> &Lep_Dz,TTreeReaderArray<Float_t> &Lep_sip3d,TTreeReaderArray<Float_t> &Lep_MiniPFRelIso,TTreeReaderArray<Int_t> &Lep_jetIdx,TTreeReaderArray<Float_t> &Jet_btagDeepFlavB,TTreeReaderArray<Float_t> &Lep_mvaTTH,UInt_t nlLep,float lPtTightCut,float lPtLooseCut,float lEtaCut,float lDxyCut,float lDzCut,float lMiniPFRelIsoCut,float lepJetDeepTagLooseCut,float lepJetDeepTagMediumCut,float lPromptMVACut,float lSIP3DCut,UInt_t &muTightCount,UInt_t &muLooseCount,bool debug){
    for (UInt_t lItr=0; lItr < nlLep; lItr++){
        if (debug) std::cout << "lItr " << lItr<< "\n";
        bool lepPassTight = false;
        bool lepPassLoose = false;
                    ...
                    if (tmpLepSIP3D < lSIP3DCut) {
                        ...
                        if (tmpLep_MiniPFRelIso < lMiniPFRelIsoCut*tmpPt) {
                            ...
                            if (Lep_LooseIdL[lItr]){
                                if (debug) std::cout << "passed loose muon ID\n";
                                ...
                                if (tmpPt > lPtLooseCut){
                                    ...
                                    if (Lep_MediumIdL[lItr]){
                                        if (debug) std::cout << "passed tight muon ID\n";
                                        if (tmpPt > lPtTightCut){
                                            if (debug) std::cout << "passed tight pt cut\n";
...
void getNTightAndLooseMuons_RochUnc(TTreeReaderArray<Bool_t> &Lep_LooseIdL,TTreeReaderArray<Bool_t> &Lep_MediumIdL,TTreeReaderArray<Float_t> &Lep_eta,std::vector<Float_t> &Lep_pt,TTreeReaderArray<Float_t> &Lep_Dxy,TTreeReaderArray<Float_t> &Lep_Dz,TTreeReaderArray<Float_t> &Lep_sip3d,TTreeReaderArray<Float_t> &Lep_MiniPFRelIso,TTreeReaderArray<Int_t> &Lep_jetIdx,TTreeReaderArray<Float_t> &Jet_btagDeepFlavB,TTreeReaderArray<Float_t> &Lep_mvaTTH,UInt_t nlLep,float lPtTightCut,float lPtLooseCut,float lEtaCut,float lDxyCut,float lDzCut,float lMiniPFRelIsoCut,float lepJetDeepTagLooseCut,float lepJetDeepTagMediumCut,float lPromptMVACut,float lSIP3DCut,UInt_t &muTightCount,UInt_t &muLooseCount,bool debug){
    for (UInt_t lItr=0; lItr < nlLep; lItr++){
        if (debug) std::cout << "lItr " << lItr<< "\n";
        bool lepPassTight = false;
        bool lepPassLoose = false;
...
void dolCandCutWithID_RochUnc(TTreeReaderArray<Bool_t> &Lep_IdL,TTreeReaderArray<Float_t> &Lep_eta,TTreeReaderArray<Float_t> &Lep_mass,TTreeReaderArray<Int_t> &Lep_charge,TTreeReaderArray<Float_t> &Lep_phi,std::vector<Float_t> &Lep_pt,TTreeReaderArray<Float_t> &Lep_Dxy,TTreeReaderArray<Float_t> &Lep_Dz,TTreeReaderArray<Float_t> &Lep_sip3d,TTreeReaderArray<Float_t> &Lep_MiniPFRelIso,TTreeReaderArray<Float_t> &Electron_sieie,TTreeReaderArray<Float_t> &Electron_hoe,TTreeReaderArray<Float_t> &Electron_eInvMinusPInv,TTreeReaderArray<Bool_t> &Electron_convVeto,TTreeReaderArray<UChar_t> &Electron_lostHits,TTreeReaderArray<Int_t> &Lep_jetIdx,TTreeReaderArray<Float_t> &Jet_btagDeepFlavB,TTreeReaderArray<Float_t> &Lep_mvaTTH,UInt_t nlLep,std::vector<UInt_t> &lepCandIndAr,std::vector<ROOT::Math::PtEtaPhiMVector> &lepCandVecAr,std::vector<Int_t> &lepCandChargeAr,float lPtCut,float lEtaCut,float lDxyCut,float lDzCut,float lMiniPFRelIsoCut,float eSieieBarrelCut,float eSieieEndcapCut,float eHoeCut,float eInvMinusPInvCut,UInt_t eLostHitsCut,float lepJetDeepTagLooseCut,float lepJetDeepTagMediumCut,float lPromptMVACut,float lSIP3DCut,bool isMuon,bool debug){
    for (UInt_t lItr=0; lItr < nlLep; lItr++){
        if (debug) std::cout << "lItr " << lItr<< "Lep_IdL[lItr]" << Lep_IdL[lItr] << "\n";
        bool lepPass = false;
        if (!Lep_IdL[lItr]){
...
void doBJetVeto_WithBDTVariables_JECUnc(UInt_t nJetLen,std::vector<Float_t> &Jet_pt,TTreeReaderArray<Float_t> &Jet_eta,TTreeReaderArray<Float_t> &Jet_phi,TTreeReaderArray<Float_t> &Jet_btagDeepFlavB,float &hFatJet_phi_fromHParticleNet,float &hFatJet_eta_fromHParticleNet,float dRCut,float bTagCut,bool &passesCut,float &maxPassingBTag,bool debug){
    //debug = true;
    //std::cout << "++++++++ "<< VBFJetdRCut << "\n";
    passesCut = true;
    for (UInt_t jetInd=0; jetInd<nJetLen-1;jetInd++){
...
void doVBFJetCutPtSelectionWithBDTVars(UInt_t nJetLen,TTreeReaderArray<Float_t> &Jet_pt,TTreeReaderArray<Int_t> &Jet_jetId,TTreeReaderArray<Float_t> &Jet_eta,TTreeReaderArray<Float_t> &Jet_phi,TTreeReaderArray<Float_t> &Jet_mass,float jetPTCut,float jetEtaDifCut,float jetInvMassCut,float &jetPairInvMass,float &jetLeadPt,float &jetLeadEta,float &jetLeadPhi,float &jetLeadMass,float &jetTrailingPt,float &jetTrailingEta,float &jetTrailingPhi,float &jetTrailingMass,UInt_t &leadJet_1,UInt_t &leadJet_2,std::vector<ROOT::Math::PtEtaPhiMVector> dRCheckVecAr,float dRCut,bool debug){
    //debug = true;
    //std::cout << "++++++++ "<< VBFJetdRCut << "\n";
    for (UInt_t jetIndOne=0; jetIndOne<nJetLen-1;jetIndOne++){
        float jetPtOne = Jet_pt[jetIndOne];
        ...
        for (UInt_t jetIndTwo=jetIndOne+1; jetIndTwo<nJetLen; jetIndTwo++){
            ...
            if (passPt){
                ...
                if (jetPtOne >= jetPtTwo) {
                    ...
                    jetLeadEta = jetEtaOne;
                    jetLeadPhi = jetPhiOne;
                    jetLeadMass = jetMassOne;
                    jetTrailingPt = jetPtTwo;
                    jetTrailingEta = jetEtaTwo;
                    jetTrailingPhi = jetPhiTwo;
                    jetTrailingMass = jetMassTwo;
                    leadJet_1 = jetIndOne;
                    leadJet_2 = jetIndTwo;
                }
                else {
                    ...
                    jetLeadEta = jetEtaTwo;
                    jetLeadPhi = jetPhiTwo;
                    jetLeadMass = jetMassTwo;
                    jetTrailingPt = jetPtOne;
                    jetTrailingEta = jetEtaOne;
                    jetTrailingPhi = jetPhiOne;
                    jetTrailingMass = jetMassOne;
                    leadJet_1 = jetIndTwo;
                    leadJet_2 = jetIndOne;
                }
            }
...
void doVBFJetCutdEtaSelectionWithBDTVars(UInt_t nJetLen,TTreeReaderArray<Float_t> &Jet_pt,TTreeReaderArray<Int_t> &Jet_jetId,TTreeReaderArray<Float_t> &Jet_eta,TTreeReaderArray<Float_t> &Jet_phi,TTreeReaderArray<Float_t> &Jet_mass,float jetPTCut,float jetEtaDifCut,float jetInvMassCut,float &jetPairInvMass,float &jetLeadPt,float &jetLeadEta,float &jetLeadPhi,float &jetLeadMass,float &jetTrailingPt,float &jetTrailingEta,float &jetTrailingPhi,float &jetTrailingMass,UInt_t &leadJet_1,UInt_t &leadJet_2,std::vector<ROOT::Math::PtEtaPhiMVector> dRCheckVecAr,float dRCut,bool debug){
    //debug = true;
    //std::cout << "++++++++ "<< VBFJetdRCut << "\n";
    float jetEtaDifMax = 0;
    for (UInt_t jetIndOne=0; jetIndOne<nJetLen-1;jetIndOne++){
        float jetPtOne = Jet_pt[jetIndOne];
        Int_t jetIdOne = Jet_jetId[jetIndOne];
        if (debug) std::cout << "jetIndOne: " << jetIndOne << " Jet_pt[jetIndOne]: " << Jet_pt[jetIndOne] << " Jet_jetId[jetIndOne]: " << Jet_jetId[jetIndOne] << "\n";
        ...
        for (UInt_t jetIndTwo=jetIndOne+1; jetIndTwo<nJetLen; jetIndTwo++){
            ...
            //Checking dEta for jet selection
            bool passdEta = false;
            if (tmpJetEtaDif > jetEtaDifMax) {
                passdEta = true;
                jetEtaDifMax = tmpJetEtaDif;
            }
            if (passdEta){
                jetPairInvMass = tmpJetPairInvMass;
                if (jetPtOne >= jetPtTwo) {
...
void doVBFJetCutdEtaSelectionNoInvMassWithBDTVars(UInt_t nJetLen,TTreeReaderArray<Float_t> &Jet_pt,TTreeReaderArray<Int_t> &Jet_jetId,TTreeReaderArray<Float_t> &Jet_eta,TTreeReaderArray<Float_t> &Jet_phi,TTreeReaderArray<Float_t> &Jet_mass,float jetPTCut,float jetEtaDifCut,float jetInvMassCut,float &jetPairInvMass,float &jetLeadPt,float &jetLeadEta,float &jetLeadPhi,float &jetLeadMass,float &jetTrailingPt,float &jetTrailingEta,float &jetTrailingPhi,float &jetTrailingMass,UInt_t &leadJet_1,UInt_t &leadJet_2,std::vector<ROOT::Math::PtEtaPhiMVector> dRCheckVecAr,float dRCut,bool debug){
    //debug = true;
    //std::cout << "++++++++ "<< VBFJetdRCut << "\n";
    float jetEtaDifMax = 0;
    for (UInt_t jetIndOne=0; jetIndOne<nJetLen-1;jetIndOne++){
...
void doVBFJetCutdEtaSelectionNoInvMassWithBDTVar_JECUnc(UInt_t nJetLen,std::vector<Float_t> &Jet_pt,TTreeReaderArray<Int_t> &Jet_jetId,TTreeReaderArray<Float_t> &Jet_eta,TTreeReaderArray<Float_t> &Jet_phi,TTreeReaderArray<Float_t> &Jet_mass,float jetPTCut,float jetEtaDifCut,float jetInvMassCut,float &jetPairInvMass,float &jetLeadPt,float &jetLeadEta,float &jetLeadPhi,float &jetLeadMass,float &jetTrailingPt,float &jetTrailingEta,float &jetTrailingPhi,float &jetTrailingMass,UInt_t &leadJet_1,UInt_t &leadJet_2,std::vector<ROOT::Math::PtEtaPhiMVector> dRCheckVecAr,float dRCut,bool debug){
    //debug = true;
    //std::cout << "++++++++ "<< VBFJetdRCut << "\n";
    float jetEtaDifMax = 0;
    for (UInt_t jetIndOne=0; jetIndOne<nJetLen-1;jetIndOne++){
...
void doHiggsFatJetCutBeforeZMDPNSelection(UInt_t nFatJetLen,float &hFatJet_pt_fromHParticleNet,float &hFatJet_phi_fromHParticleNet,float &hFatJet_eta_fromHParticleNet,float &hFatJet_mass_fromHParticleNet,float &hFatJet_HParticleNet_fromHParticleNet,UInt_t &hFatJet_ind_fromHParticleNet,TTreeReaderArray<Float_t> &particleNetMD_QCDL,TTreeReaderArray<Float_t> &particleNetMD_XbbL,float hFatJetPNCut,TTreeReaderArray<Float_t> &FatJet_pt,float hFatJetPTCut,TTreeReaderArray<Int_t> &FatJet_jetId,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,std::vector<ROOT::Math::PtEtaPhiMVector> &dRCheckVecAr,float dRCut,bool passedAsSemiLepLepCutBool,bool &passedAsSemiLepHiggsCutBool){
    if (passedAsSemiLepLepCutBool){
        float tmpFJHParticleNetMax = hFatJetPNCut;
        ROOT::Math::PtEtaPhiMVector HFJVec;
        for (UInt_t fatJetInd=0;fatJetInd<nFatJetLen;fatJetInd++){
            ...
            if (debug) {
                std::cout << fatJetInd << " +++++++++++++++\n";
                std::cout << "Fat jet HPNMD    " <<  HPNMD << "\n";
            }
            
            if (HPNMD > tmpFJHParticleNetMax){
                float tmpFatJet_pt = FatJet_pt[fatJetInd];
                ...
                if (tmpFatJet_pt > hFatJetPTCut){
                    ...
                    if (tmpFatJet_jetId == 6){
                        ...
                        if (HPNMD > hFatJet_HParticleNet_fromHParticleNet){
                            if (debug) std::cout << "HParticleNet greater than previous FJ HParticleNet\n";
                            float tmpFatJet_phi = FatJet_phi[fatJetInd];
                            float tmpFatJet_eta = FatJet_eta[fatJetInd];
                            //ROOT::Math::PtEtaPhiMVector tmpFJVec = ROOT::Math::PtEtaPhiMVector(tmpFatJet_pt,tmpFatJet_eta,tmpFatJet_phi,FatJet_mass[fatJetInd]);
                            ...
                            if (tmpPassdR) {
                                ...
                                hFatJet_HParticleNet_fromHParticleNet = HPNMD;
                                hFatJet_pt_fromHParticleNet = tmpFatJet_pt;
                                hFatJet_eta_fromHParticleNet = tmpFatJet_eta;
                                hFatJet_phi_fromHParticleNet = tmpFatJet_phi;
                                hFatJet_mass_fromHParticleNet = tmpFatJet_mass;
                                hFatJet_ind_fromHParticleNet = fatJetInd;
                                tmpFJHParticleNetMax = HPNMD;
                                HFJVec = tmpFJVec;
        ...
        if (hFatJet_pt_fromHParticleNet != 0) {
            //HiggsFJInd = hFatJet_ind_fromHParticleNet);
            dRCheckVecAr.push_back(HFJVec);
            passedAsSemiLepHiggsCutBool = true;
        }
    }
}

void doHiggsFatJetCutBeforeZMDPNSelection_JECUnc(UInt_t nFatJetLen,float &hFatJet_pt_fromHParticleNet,float &hFatJet_phi_fromHParticleNet,float &hFatJet_eta_fromHParticleNet,float &hFatJet_mass_fromHParticleNet,float &hFatJet_HParticleNet_fromHParticleNet,UInt_t &hFatJet_ind_fromHParticleNet,TTreeReaderArray<Float_t> &particleNetMD_QCDL,TTreeReaderArray<Float_t> &particleNetMD_XbbL,float hFatJetPNCut,std::vector<Float_t> &FatJet_pt,float hFatJetPTCut,TTreeReaderArray<Int_t> &FatJet_jetId,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,std::vector<ROOT::Math::PtEtaPhiMVector> &dRCheckVecAr,float dRCut,bool passedAsSemiLepLepCutBool,bool &passedAsSemiLepHiggsCutBool){
    if (passedAsSemiLepLepCutBool){
        float tmpFJHParticleNetMax = hFatJetPNCut;
        ROOT::Math::PtEtaPhiMVector HFJVec;
        for (UInt_t fatJetInd=0;fatJetInd<nFatJetLen;fatJetInd++){
...
void doHiggsFatJetCutEarlyMDPNSelection_JECUnc(UInt_t nFatJetLen,float &hFatJet_pt_fromHParticleNet,float &hFatJet_phi_fromHParticleNet,float &hFatJet_eta_fromHParticleNet,float &hFatJet_mass_fromHParticleNet,float &hFatJet_HParticleNet_fromHParticleNet,UInt_t &hFatJet_ind_fromHParticleNet,TTreeReaderArray<Float_t> &particleNetMD_QCDL,TTreeReaderArray<Float_t> &particleNetMD_XbbL,float hFatJetPNCut,std::vector<Float_t> &FatJet_pt,float hFatJetPTCut,TTreeReaderArray<Int_t> &FatJet_jetId,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,std::vector<ROOT::Math::PtEtaPhiMVector> &dRCheckVecAr,float dRCut,std::vector<Int_t> &FJIndAr){
    float tmpFJHParticleNetMax = hFatJetPNCut;
    ROOT::Math::PtEtaPhiMVector HFJVec;
    for (UInt_t fatJetInd=0;fatJetInd<nFatJetLen;fatJetInd++){
        bool sameFJ = false;
...
void doSemiLepChanFatJetCutAfterHMDPNSelection(Int_t &FJInd,Int_t numFatJet,float fatJetPTCut,float fatJetZTagCut,TTreeReaderArray<Float_t> &FatJet_pt,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,TTreeReaderArray<Float_t> &particleNetMD_QCDL,TTreeReaderArray<Float_t> &particleNetMD_XbbL,TTreeReaderArray<Float_t> &particleNetMD_XccL,TTreeReaderArray<Float_t> &particleNetMD_XqqL,TTreeReaderArray<Int_t> &FatJet_jetId,
std::vector<ROOT::Math::PtEtaPhiMVector> &dRCheckVecAr,float dRCut, UInt_t hFatJet_ind_fromHTag, bool passedAsSemiLepHiggsCutBool,
UInt_t &passSemiLepCut,bool &passesCutsBool,bool &passedAsSemiLepBool){
    ...
    if (dRCheckVecAr.size() && passedAsSemiLepHiggsCutBool){
        for (UInt_t i=0; i<numFatJet;i++){
            ...
            float ZPNMD = (particleNetMD_XbbL[i]+particleNetMD_XccL[i]+particleNetMD_XqqL[i])/(particleNetMD_XbbL[i]+particleNetMD_XccL[i]+particleNetMD_XqqL[i]+particleNetMD_QCDL[i]);
            
                    
            //PN MD For Z used for Selection
            if (ZPNMD > fatJetZTagCut && tmpFatJetPT > fatJetPTCut && ZPNMD > semiLepFatJetTag && tmpFatJet_jetId == 6){
...
void doSemiLepChanFatJetCutAfterHMDPNSelection_JECUnc(Int_t &FJInd,Int_t numFatJet,float fatJetPTCut,float fatJetZTagCut,std::vector<Float_t> &FatJet_pt,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,TTreeReaderArray<Float_t> &particleNetMD_QCDL,TTreeReaderArray<Float_t> &particleNetMD_XbbL,TTreeReaderArray<Float_t> &particleNetMD_XccL,TTreeReaderArray<Float_t> &particleNetMD_XqqL,TTreeReaderArray<Int_t> &FatJet_jetId,
std::vector<ROOT::Math::PtEtaPhiMVector> &dRCheckVecAr,float dRCut, UInt_t hFatJet_ind_fromHTag, bool passedAsSemiLepHiggsCutBool,
UInt_t &passSemiLepCut,bool &passesCutsBool,bool &passedAsSemiLepBool){
...
void doSemiLepChanFatJetCutMDPNSelection(Int_t &FJInd,Int_t numFatJet,float fatJetPTCut,float fatJetZTagCut,TTreeReaderArray<Float_t> &FatJet_pt,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,TTreeReaderArray<Float_t> &particleNetMD_QCDL,TTreeReaderArray<Float_t> &particleNetMD_XbbL,TTreeReaderArray<Float_t> &particleNetMD_XccL,TTreeReaderArray<Float_t> &particleNetMD_XqqL,TTreeReaderArray<Int_t> &FatJet_jetId,
...
void doHadChanFatJetCut_JECUnc(Int_t &LFJOneInd,Int_t &LFJTwoInd,Int_t numFatJet,float fatJetPTCut,float fatJetZPairInvMassCut,std::vector<Float_t> &FatJet_pt,TTreeReaderArray<Float_t> &FatJet_phi,TTreeReaderArray<Float_t> &FatJet_eta,TTreeReaderArray<Float_t> &FatJet_mass,TTreeReaderArray<Float_t> &FatJet_deepTag_ZvsQCD){
...
void dolZ1CutWithTree_RochUnc(std::vector<Float_t> &Lep_pt,std::vector<UInt_t> lepCandIndAr,std::vector<ROOT::Math::PtEtaPhiMVector> lepCandVecAr,std::vector<Int_t> lepCandChargeAr,std::vector<std::array<UInt_t,2>> &lepPassesZ2CutsAr,bool &Z1IsMuon,bool forMuon,float invMassCutLow,float invMassCutHigh,float ptLeadCut,float ptTrailingCut,float &tmpTopZ1LeadPt,float &tmpTopZ1TrailingPt,Int_t &tmpTopLeadItr,Int_t &tmpTopTrailingItr,ROOT::Math::PtEtaPhiMVector &tmpTopLeadVec,ROOT::Math::PtEtaPhiMVector &tmpTopTrailingVec,Int_t &tmpTopLeadCharge,Int_t &tmpTopTrailingCharge,float &difFromZMassOne,std::vector<Float_t> &SemiLepOrLepInvMass,bool debug){
    if (debug) std::cout << "in dolZ1Cut fcn\n";
    for (UInt_t lItrOne=0; lItrOne<lepCandIndAr.size()-1; lItrOne++) { //LEP LOOP ONE
        Int_t tmpChargeOne = lepCandChargeAr[lItrOne];
        if (debug) std::cout << "lItrOne " << lItrOne << " lepCandIndAr[lItrOne] " << lepCandIndAr[lItrOne] << " tmpChargeOne " << tmpChargeOne << "\n";
...
void doLeptonicCutsWithTree_RochUnc(TTreeReaderArray<Float_t> &Electron_etaL,TTreeReaderArray<Float_t> &Electron_massL,TTreeReaderArray<Int_t> &Electron_chargeL,TTreeReaderArray<Float_t> &Electron_phiL,TTreeReaderArray<Float_t> &Electron_ptL,UInt_t neLep,std::vector<UInt_t> &elecCandIndAr,std::vector<ROOT::Math::PtEtaPhiMVector> &elecCandVecAr,std::vector<Int_t> &elecCandChargeAr,UInt_t &negElecCands,UInt_t &posElecCands,UInt_t &totElecCands,bool &enoughElecCands,UInt_t &negMuonCands,UInt_t &posMuonCands,UInt_t &totMuonCands,bool &enoughMuonCands,bool &enoughLepCands,float ePtCut,float eEtaCut,
TTreeReaderArray<Float_t> &Muon_etaL,TTreeReaderArray<Float_t> &Muon_massL,TTreeReaderArray<Int_t> &Muon_chargeL,TTreeReaderArray<Float_t> &Muon_phiL,std::vector<Float_t> &Muon_ptL,UInt_t nmLep,std::vector<UInt_t> &muonCandIndAr,std::vector<ROOT::Math::PtEtaPhiMVector> &muonCandVecAr,std::vector<Int_t> &muonCandChargeAr,
...
void doSemiLepCutWithTreeWithBDTVars_RochUnc(bool &enoughElecCands,UInt_t &negElecCands,UInt_t &posElecCands,UInt_t &totElecCands,TTreeReaderArray<Float_t> &Electron_etaL,TTreeReaderArray<Float_t> &Electron_massL,TTreeReaderArray<Int_t> &Electron_chargeL,TTreeReaderArray<Float_t> &Electron_phiL,TTreeReaderArray<Float_t> &Electron_ptL,UInt_t neLep,std::vector<UInt_t> &elecCandIndAr,std::vector<ROOT::Math::PtEtaPhiMVector> &elecCandVecAr,std::vector<Int_t> &elecCandChargeAr,float ePtCut,float eEtaCut,
bool &enoughMuonCands,UInt_t &negMuonCands,UInt_t &posMuonCands,UInt_t &totMuonCands,TTreeReaderArray<Float_t> &Muon_etaL,TTreeReaderArray<Float_t> &Muon_massL,TTreeReaderArray<Int_t> &Muon_chargeL,TTreeReaderArray<Float_t> &Muon_phiL,std::vector<Float_t> &Muon_ptL,UInt_t nmLep,std::vector<UInt_t> &muonCandIndAr,std::vector<ROOT::Math::PtEtaPhiMVector> &muonCandVecAr,std::vector<Int_t> &muonCandChargeAr,
...
bool debug){
    ...
    if (enoughLepCands){ //Check there are enough lep cands after candidate cuts
        ...
        std::vector<std::array<UInt_t,2>> muonPassesZ2CutsAr;
        if (enoughMuonCands){ //If enough muon cands, run Z1 cuts
            dolZ1CutWithTree_RochUnc(Muon_ptL,muonCandIndAr,muonCandVecAr,muonCandChargeAr,muonPassesZ2CutsAr,Z1IsMuon,true,invMassCutLow,invMassCutHigh,ptLeadCut,ptTrailingCut,Z1LeadPt,Z1TrailingPt,Z1LeadItr,Z1TrailingItr,Z1LeadVec,Z1TrailingVec,Z1LeadCharge,Z1TrailingCharge,difFromZMassOne,SemiLepInvMass,debug);
        }
        if (debug) std::cout <<"Z1LeadItr " << Z1LeadItr << " Z1TrailingItr " << Z1TrailingItr << " Z1LeadPt " << Z1LeadPt << " Z1TrailingPt " << Z1TrailingPt << " Z1LeadVec " << Z1LeadVec << " Z1TrailingVec " << Z1TrailingVec << " Z1LeadCharge " << Z1LeadCharge << " Z1TrailingCharge " << Z1TrailingCharge << " Z1IsMuon " << Z1IsMuon << " difFromZMassOne " << difFromZMassOne << "\n";
        //if (Z1LeadItr<0) {
        //    std::cout << "ddntpass Z1LeadItr" << Z1LeadItr << "\n";
