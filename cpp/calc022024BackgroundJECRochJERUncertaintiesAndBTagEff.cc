...
void calc022024BackgroundJECRochJERUncertaintiesAndBTagEff(string datasetString, int JECCorInd, bool JECCorUpOrDown,  int RochInd, int JERInd,uint yearInd){
    ...
    if (ttHToBBBackground){
        ...
        for (int i=0; i<arrSize; i++){
            fileAr.push_back(strAdd+"HLTTrimmedFilteredForAnalysis"+saveName+yearStr+"_WithJERRoch.root");
    ...
    */
    ...
    std::ofstream btagOutFile("btag_eff_"+saveName+yearStr+std::to_string(JECCorInd)+"_"+std::to_string(JECCorUpOrDown)+"_"+std::to_string(RochInd)+"_"+std::to_string(JERInd)+".py");
