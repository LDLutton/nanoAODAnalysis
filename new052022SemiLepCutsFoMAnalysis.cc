...
#include "new052022FoMAnalysis.h"
#include "new052022FoMAnalysis_Functions.h"
...
void new052022SemiLepCutsFoMAnalysis(string inputString){
    ...
    cutString = elems[1];
    std::cout << "start SemiLep " << cutString <<  "Cut FoM Analysis\n";
    ...
    if (cutString == "dR") {
        cutRangeBottom = dRCutRangeBottom;
        cutStep = dRCutCutStep;
        cutPtr = &dRCut;
    };
    if (cutString == "fatJetZTag") {
        cutRangeBottom = fatJetZTagRangeBottom;
        cutStep = fatJetZTagCutStep;
        cutPtr = &fatJetZTagCut;
    };
    if (cutString == "fatJetPT") {
        cutRangeBottom = fatJetPTRangeBottom;
        cutStep = fatJetPTCutStep;
        cutPtr = &fatJetPTCut;
    };
...
/*
    if (debug) {
        std::cout << "eEtaCut " << eEtaCut << "\n";
        std::cout << "cutPtr " << cutPtr << "\n";
        std::cout << "*cutPtr " << *cutPtr << "\n";
        *cutPtr = 3.;
        std::cout << "eEtaCut " << eEtaCut << "\n";
        std::cout << "cutPtr " << cutPtr << "\n";
...
*/
    //std::cout << "fatJetZTagCut " << fatJetZTagCut << " fatJetZTagRangeBottom " << fatJetZTagRangeBottom << " fatJetZTagCutStep " << fatJetZTagCutStep << "\n";
    //std::cout << "*cutPtr " << *cutPtr << " cutRangeBottom " << cutRangeBottom << " cutStep " << cutStep << "\n";
    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        ...
        //std::cout << "cutRangeAr[rangeItr] " << cutRangeAr[rangeItr] << " cutRangeBottom+(rangeItr*cutStep) "<< cutRangeBottom+(rangeItr*cutStep) <<  "\n";
    }
/*
    if (debug) {
        std::cout << "eEtaCut " << eEtaCut << "\n";
    ...
    */
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            if (debug){
                std::cout << "-------------------------\n";
                std::cout << evCount+1 << " starting lep loop\n";
            }
            ...
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                ...
                *cutPtr = cutRangeAr[rangeItr];
                ...
                if ((!passesCutsBool)){
                    ...
                    //std::cout << "fatJetZTagCut " << fatJetZTagCut << "\n";
                    doSemiLepChanFatJetCut(FJInd,numFatJet,fatJetPTCut,fatJetZTagCut,FatJet_ptL,FatJet_phiL,FatJet_etaL,FatJet_massL,FatJet_deepTag_ZvsQCDL,dRCheckVecAr,dRCut,passSemiLepCut,passesCutsBool,passedAsSemiLepBool);
                    if (debug){
                    ...
                    //std::cout << "passesCutsBool " << passesCutsBool << " passedAsSemiLepBool " << passedAsSemiLepBool <<" FJInd " << FJInd << " FatJet_deepTag_ZvsQCDL[FJInd] " << FatJet_deepTag_ZvsQCDL[FJInd] <<"\n";
                ...
                doHiggsFatJetCut(nFatJetLen,hFatJet_pt_fromHTag,hFatJet_phi_fromHTag,hFatJet_eta_fromHTag,hFatJet_mass_fromHTag,hFatJet_HTag_fromHTag,hFatJet_ind_fromHTag,FatJet_deepTag_HL,hFatJetDeepTagCut,FatJet_ptL,hFatJetPTCut,FatJet_jetIdL,FatJet_phiL,FatJet_etaL,FatJet_massL,dRCheckVecAr,dRCut,FJIndAr,ZPairPlusHInvMassCut,ZPairPlusHPtCut);
