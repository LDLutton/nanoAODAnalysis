...
void new032022InvMassCutLowCutFoMAnalysis(string datasetString){
    // Open the file. Note that the name of your file outside this class
    // will probably NOT be experiment.root.
    std::cout << "start invMassCutLowCut FoM Analysis\n";
    ...
    for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
        ...
        cutRangeAr.push_back(invMassCutLowRangeBottom+(rangeItr*invMassCutLowCutStep));
    }
    ...
    for (UInt_t k=0; k<fileAr.size(); k++){
        ...
        while (myEventsReader.Next()) {
            ...
            for (UInt_t rangeItr=0; rangeItr<cutAmnt+1; rangeItr++){
                ...
                float tmpinvMassCutLowCutVal = cutRangeAr[rangeItr];
                ...
                Z1IsMuon,tmpinvMassCutLowCutVal,invMassCutHigh,ptLeadCut,ptTrailingCut,
                ...
                if (rangeItr <10){
                    std::cout << "-----------------------------------";
                    std::cout <<  "evCount " << evCount << " rangeItr "  << rangeItr<< " enoughLepCands " << enoughLepCands << " difFromZMassOne " << difFromZMassOne << " enoughLepCands "<<enoughLepCands << " passedAsLepBool " << passedAsLepBool << " passesCutsBool " << passesCutsBool << "\n";
                    std::cout <<  "Z1IsMuon " << Z1IsMuon << " tmpinvMassCutLowCutVal "  << tmpinvMassCutLowCutVal<< " Z1LeadPt " << Z1LeadPt << " Z1LeadItr " << Z1LeadItr << " Z1LeadPt "<<Z1LeadPt << " Z1LeadSIP " << Z1LeadSIP << " tmpTopZ2LeadPt " << tmpTopZ2LeadPt << " tmpZ2Ind " << tmpZ2Ind <<  "\n";
                    std::cout << "Z1LeadIso " << Z1LeadIso<< " Z1TrailingIso " << Z1TrailingIso << " Z2LeadIso " << Z2LeadIso << " Z2TrailingIso " << Z2TrailingIso << " lepIsoCut " << lepIsoCut << "\n";
                    std::cout <<  "Z2IsMuon " << Z2IsMuon << " tmpTopZ2TrailingPt " << tmpTopZ2TrailingPt << " Z2LeadSIP "<<Z2LeadSIP << " Z2TrailingSIP " << Z2TrailingSIP << "\n";
                    std::cout << "neLep " << neLep<< " nmLep " << nmLep << " totElecCands " << totElecCands << " totMuonCands " << totMuonCands << " lepIsoCut " << lepIsoCut << "\n";
                }
                */
                ...
                */
                ...
                if (passesCutsBool){
                    ...
                    if (passedAsLepBool){
                        /*
                        if ( rangeItr <10){
                            std::cout << "WOW IT PASSED " << rangeItr << "\n";
                        }
                        */
